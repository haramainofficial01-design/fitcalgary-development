...
def page5(c):
    ...
    txt(c, 42, 620, "WHAT IS COMPLETE", 8, CORAL, True); wrap_text(c, 42, 600, "The Phase 2 core product is materially complete and verified in the local development/staging environment. Ramy's approved catalog is imported through the production-shaped data model and visible in the product.", 360, 9.5, 13, INK)
    ...
    c.setFillColor(CORAL); c.roundRect(42, 176, 528, 108, 10, fill=1, stroke=0); txt(c, 60, 254, "PHASE 2 - READY FOR CLIENT REVIEW", 12, WHITE, True); wrap_text(c, 60, 231, "This package shows the working product, Client-approved catalog, tested service path and precise boundaries that remain external or belong to final release preparation. Client acceptance is not assumed.", 470, 9, 13, WHITE); txt(c, 60, 194, "Final Client-data integration checkpoint recorded in private Git history.", 7.5, colors.HexColor("#F7DDD7"), True); footer(c, 5); c.showPage()
