...
def page1(c):
    ...
    wrap_text(c, 42, H - 166, "A working FitCalgary product layer built on the tested Flutter, Go and PostgreSQL foundation.", 250, 10.5, 15, colors.HexColor("#D8D9D6")); pill(c, 42, H - 230, "READY FOR CLIENT REVIEW", CORAL, 142); txt(c, 42, H - 258, "September 2026  |  acceptance candidate", 7.5, colors.HexColor("#BFC1BD"))
    ...
    cards = [("Flutter / Dart", "VERIFIED", "20 tests + analysis"), ("Go services", "VERIFIED", "tests / vet / build"), ("PostgreSQL", "VERIFIED", "7 migrations / 35 tables"), ("Auth + roles", "TESTED", "server enforced"), ("iOS", "SIMULATOR", "build + launch"), ("Android", "EMULATOR", "build + AAB"), ("Web", "VERIFIED", "type / lint / build"), ("Apple Watch", "SIMULATOR", "build + launch")]
    for i, (t, v, d) in enumerate(cards): status_card(c, 42 + (i % 4) * 132, 288 - (i // 4) * 70, 122, t, v, d)
    txt(c, 42, 134, "CENTRAL ACCEPTANCE PATH", 8, CORAL, True); txt(c, 42, 112, "submission  ->  private evidence  ->  correction or approval  ->  verified result  ->  official board  ->  notification", 8.3, INK, True); wrap_text(c, 42, 86, "Development fixtures are isolated from production content. Physical devices, live provider credentials, approved Client data and store submission remain outside this local verification claim.", 525, 8.2, 11, MUTED); footer(c, 1); c.showPage()
...
def page2(c):
    ...
    image_card(c, ART / "phase2/quality/clean/gym-detail.png", 42, 378, 164, 285, "Gym detail + saved state"); image_card(c, ART / "phase2/quality/clean/comparison.png", 224, 378, 164, 285, "Normalized price comparison"); image_card(c, ART / "phase2-day4/day4-athlete-performance.png", 406, 378, 164, 285, "Athlete profile + history")
    image_card(c, ART / "phase2/quality/clean/private-evidence.png", 42, 104, 164, 245, "Submission + private evidence"); image_card(c, ART / "phase2/quality/clean/official-result.png", 224, 104, 164, 245, "Verified official placement"); image_card(c, ART / "phase2/quality/clean/event-editor-mobile.png", 406, 104, 164, 245, "Clubs / events operations"); footer(c, 2); c.showPage()
...
def page3(c):
    ...
    node(c, 137, 248, 170, 82, "POSTGRESQL", "35 public tables, 7 migrations"); node(c, 353, 248, 170, 82, "PRIVATE STORAGE", "ownership checks + signed playback"); arrow(c, 222, 386, 222, 330); arrow(c, 438, 386, 438, 330)
    ...
    txt(c, 42, 62, "No production secrets are committed. Provider credentials, final content and physical-device/store verification are explicitly separated from local evidence.", 7.5, MUTED); footer(c, 3); c.showPage()
...
def page4(c):
    ...
    groups = [("COMPLETE + VERIFIED", GREEN, ["1  Account, profile, saved gym and reload", "2  Directory, filters and structured comparison", "3  Admin publish to client visibility", "4  Community result to board and profile", "5  Submission -> evidence -> correction -> approval -> rank -> inbox", "6  Role/object denial and immediate revocation", "7  Moderation, preferences and notification outbox", "8  Shared client data path and Watch companion"]), ("BLOCKED_EXTERNAL", CORAL, ["Live Keycloak / SMTP / Google / Apple credentials", "FCM/APNs provider delivery credentials", "Client-approved production content and ranking decisions", "Physical devices, signed stores, hosting and DNS"])]
    y = 598
...
def page5(c):
    ...
    txt(c, 42, 620, "WHAT IS COMPLETE", 8, CORAL, True); wrap_text(c, 42, 600, "The Phase 2 core product is materially complete and verified in the local development/staging environment. The private repository contains the acceptance candidate and the evidence package is regenerated for review.", 360, 9.5, 13, INK)
    txt(c, 42, 510, "WHAT REMAINS EXTERNAL", 8, CORAL, True); wrap_text(c, 42, 490, "Client-approved gyms, pricing, clubs, events and ranking decisions; production identity/provider credentials; hosting, storage, notification and store accounts; and physical-device access. These are dependencies, not hidden unfinished implementation.", 360, 9.5, 13, INK)
    txt(c, 42, 364, "PHASE 3 NEXT", 8, CORAL, True); wrap_text(c, 42, 344, "Release configuration, approved data import, physical-device QA, signed archives/AAB, provider verification, performance/security review, store preparation, deployment and handoff documentation.", 528, 9.5, 13, INK)
    c.setFillColor(CORAL); c.roundRect(42, 176, 528, 108, 10, fill=1, stroke=0); txt(c, 60, 254, "PHASE 2 - READY FOR CLIENT REVIEW", 12, WHITE, True); wrap_text(c, 60, 231, "This package shows the working product, the tested service path and the precise boundaries that remain external or belong to final release preparation. Client acceptance is not assumed.", 470, 9, 13, WHITE); txt(c, 60, 194, "Acceptance candidate: 3f858ba  |  final status: a35e28e", 7.5, colors.HexColor("#F7DDD7"), True); footer(c, 5); c.showPage()
