...
func (s *Server) listContent(r *http.Request, events bool) (any, error) {
	...
	if events {
		...
		conditions += ` AND ($7::text='' OR (` + eventPhaseSQL + `)=$7) AND ($8::text='' OR to_char(item.start_at AT TIME ZONE 'America/Edmonton','YYYY-MM')=$8) AND (NOT $9::boolean OR (item.registration_status='OPEN' AND item.event_status='ACTIVE' AND COALESCE(item.registration_deadline,item.start_at)>=now()))`
