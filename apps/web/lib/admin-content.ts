...
export const contentFields: Record<string,ContentField[]> = {
  ...
  events:[...identity,text('organizer','Organizer'),text('category','Category'),long('description','Description'),{key:'startAt',label:'Starts (your local time)',type:'datetime',required:true},{key:'endAt',label:'Ends (your local time)',type:'datetime'},{key:'registrationDeadline',label:'Registration deadline (your local time)',type:'datetime'},{key:'registrationStatus',label:'Registration',type:'select',options:['OPEN','CLOSED','NOT_APPLICABLE'],default:'OPEN',required:true},text('location','Location'),url('externalRegistrationUrl','Registration link'),text('sport','Sport'),{key:'officialFitcalgary',label:'Official FitCalgary event',type:'boolean'},list('tags','Tags'),url('sourceUrl','Source'),{key:'eventStatus',label:'Event status',type:'select',options:['ACTIVE','CANCELLED','POSTPONED'],default:'ACTIVE',required:true},publication,long('entryRequirements','Entry requirements'),url('imageUrl','Image URL')],
