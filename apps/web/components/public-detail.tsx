...
export function PublicDetail({ domain, id }: { domain: 'gyms' | 'events' | 'leaderboards'; id: string }) {
  ...
  const website = external(record.registration_url ?? record.website_url);
  return <>
    ...
    {website && <a className="primary-button detail-link" href={website} target="_blank" rel="noopener noreferrer">{domain === 'events' ? 'Registration details' : 'Visit website'}</a>}
  </>;
}
