...
import { eventPhaseLabel, eventRegistrationLabel } from '@/lib/event-labels';
...
export function PublicDetail({ domain, id }: { domain: 'gyms' | 'events' | 'leaderboards' | 'clubs'; id: string }) {
  ...
  return <>
    ...
    {domain === 'events' && <div className="data-panel"><h2>Event details</h2><p>{eventPhaseLabel(record.phase, !record.start_at && !!record.start_date)}{eventRegistrationLabel(record.phase, record.registration_status) ? ` · ${eventRegistrationLabel(record.phase, record.registration_status)}` : ''}</p><p>{text(record.location)}</p>{text(record.start_at) && <p><time dateTime={text(record.start_at)}>{new Date(text(record.start_at)).toLocaleString('en-CA', { timeZone: 'America/Edmonton' })} · Calgary time</time></p>}{!record.start_at && text(record.start_date) && <p><time dateTime={text(record.start_date)}>{new Date(`${text(record.start_date)}T12:00:00Z`).toLocaleDateString('en-CA', { timeZone: 'UTC', dateStyle: 'full' })}</time> · Time to be confirmed</p>}<p>{text(record.entry_requirements)}</p><p>{text(record.organizer)}</p></div>}
