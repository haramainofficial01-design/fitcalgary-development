...
| Club/event publishing services | DEVELOPMENT TESTED / CLIENT INTEGRATION PENDING | Admin create/update/publish/unpublish/archive, public detail/search/filter/paging and event temporal states use existing schema. Real PostgreSQL tests verify role boundaries, validation, visibility and atomic audit rollback. Flutter/web/admin forms and new platform workflow verification are next. |
